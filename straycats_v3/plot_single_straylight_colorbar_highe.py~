...
low_pi = utils.energy_to_chan(20)
high_pi = utils.energy_to_chan(80)
    ...
    for line in f:
        ...
        if (int(seqid) == int(inseq)):
            ...
            for ind, mod in enumerate(['A', 'B']):
                ...
                h, xi, yi = np.histogram2d(ev['DET1Y'][filt], ev['DET1X'][filt], bins = 50, range =( (0,375), (0, 375)))
                dx = xi[1] - xi[0]
                da = (dx *ns.pixel_um.to(u.cm).value)**2 * hdr['EXPOSURE']
                h = h / da
                im = ax[ind].imshow(h, aspect=1, origin='lower')

#                h, xi, yi, im = ax[ind].hist2d(ev['DET1X'][filt], ev['DET1Y'][filt], bins = 50, range =( (0,375), (0, 375)), cmin = 0, cmax = cmax)
#                if (h.max() > cmax):
#                    cmax = h.max()
  
                cbar = plt.colorbar(im, ax=ax[ind])
                cbar.set_label('cts / sec / cm2')
            plt.savefig(f'{seqid}_colorbar.pdf')
#            plt.show()
            break
